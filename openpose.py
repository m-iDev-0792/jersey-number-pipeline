...
coco17_keypoints = ['Nose', 'Leye', 'Reye', 'Lear', 'Rear', 'Lsho', 'Rsho', 'Lelb', 'Relb', 'Lwri', 'Rwri', 'Lhip',
                    'Rhip', 'Lkne', 'Rkne', 'Lank', 'Rank']
coco18_keypoints2 = ['Nose', 'Neck', 'Lsho', 'Lelb', 'Lwri', 'Rsho', 'Relb', 'Rwri', 'Lhip', 'Lkne', 'Lank', 'Rhip',
                     'Rkne', 'Rank', 'Reye', 'Leye', 'Rear', 'Lear']
coco18_keypoints = ['Nose', 'Neck', 'Rsho', 'Relb', 'Rwri', 'Lsho', 'Lelb', 'Lwri', 'Rhip', 'Rkne', 'Rank', 'Lhip',
                    'Lkne', 'Lank', 'Leye', 'Reye', 'Lear', 'Rear']
...
def main():
    ...
    if configuration.openpose_use_cache:
        print(
            'OpenPose cache will be used, if you want to regenerate poses please delete cache folders for set configuration.openpose_use_cache to False')
    ...
    for image_id in tqdm(range(len(img_keys)), desc="Check output"):
    ...
    if len(not_processed_files) > 0:
        print(
            f'There are still unprocessed images: {len(not_processed_files)} images, begin to perform pose estimation')
    ...
    ###################################
    results = []
    for image_id in tqdm(range(len(img_keys)), desc="Convert keypoints"):
        image = coco.loadImgs(image_id)[0]
        image_name = os.path.join(args.img_root, image['file_name'])
        ...
        image_basename = os.path.basename(image_name)
        filename_without_ext, extension = os.path.splitext(image_basename)
        openpose_json_path = os.path.join(out_dir, filename_without_ext + '_keypoints.json')
        # print(f'Read openpose json from {openpose_json_path} for {image_name}')
        if not os.path.exists(openpose_json_path):
        ...
        openpose_json_path = openpose_json_path.replace("\\\\", "\\")
        openpose_json_path = openpose_json_path.replace("//", "/")
    ...
    if out_json_path != '':
        with open(out_json_path, 'w') as fp:
            print(f'openpose.py: main() writing results to {out_json_path}...')
